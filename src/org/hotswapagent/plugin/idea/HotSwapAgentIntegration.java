...
public class HotSwapAgentIntegration implements StartupActivity {
    ...
    public void runActivity(final @NotNull Project project) {
        ...
        // check os version
        if (newRelease != null && ((SystemInfo.isMac)) || ((SystemInfo.isLinux) &&
                (SystemInfo.isAMD64)) || ((SystemInfo.isWindows) && (SystemInfo.is64Bit))) {
            if (jre != null) {
                application.executeOnPooledThread(
                        (Runnable) () -> checkForUpdate(project, newRelease, jre)
                );
            } else {
                new Notification(getPluginDescriptor().getName(), getPluginDescriptor().getName(),
                        "Can`t find directory with JRE for store hotswap Agent.", NotificationType.ERROR).notify(project);
            }
        } else {
            new Notification(getPluginDescriptor().getName(), getPluginDescriptor().getName(), "Hotswap Agent for current OS is unavailable.",
    ...
    private void checkForUpdate(final @NotNull Project project, final @NotNull String newRelease, final @NotNull String jre) {
        ...
        if ((!new File(jre).exists()) || (!currentRelease.equals(newRelease))) {
            if ("".equals(currentRelease)) {
                installWithAsk(project, () -> download(project, newRelease, jre));
            } else {
                downloadWithAsk(project, () -> download(project, newRelease, jre));
            }
        } else {
...
//
//        download(project, tmpFile, newRelease, jre, null);

        ProgressManager.getInstance().run(
                new Task.Backgroundable(project, "Downloading " + getPluginDescriptor().getName(), true) {
                    @Override
    ...
    public void installWithAsk(@NotNull Project project, @NotNull final Runnable callback) {
        if (Messages.showYesNoDialog(project, "Hotswap Agent", "Do You Want to Download the New Version ?",
     ...
     */
    private void download(@NotNull final Project project, final @NotNull File destination, final @NotNull String taqName, final @NotNull String jre,
            final ProgressIndicator indicator) {
        execute(project, taqName, connection -> {
            ...
            try {
//                https://github.com/HotswapProjects/HotswapAgent/releases/download/RELEASE-0.3/HotswapAgent-RELEASE-0.3.zip
//                https://github.com/HotswapProjects/HotswapAgent/releases/download/RELEASE-0.3/HotswapAgent-0.3.zip
    ...
    private void install(@NotNull Project project, @NotNull File source, @NotNull File target, String tagName) {
        ...
        try {
            ...
            extract(new ZipFile(source), target, (dir, name) -> (
     ...
     */
    private void extract(final @NotNull ZipFile zipFile, @NotNull File outputDir, @Nullable FilenameFilter filenameFilter, boolean overwrite)
            throws IOException {
    ...
    private String modifyForAlternativeJvm(String original) {
        if (SystemInfo.isLinux) {
            ...
            return original + "jre" + File.separatorChar + "bin" + File.separatorChar + GITHUB_PROJECT + File.separatorChar;
        }
     ...
     * @param jre               cesta k JVM
     */
    private void configurationTypePatchIfNecessary(final @NotNull Project project, @NotNull ConfigurationType configurationType,
            final @NotNull String jre) {
        RunManager runManager = RunManagerImpl.getInstance(project);
